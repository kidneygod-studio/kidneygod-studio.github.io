...
FACTS: dict[str, str] = {
    ...
    "beds":       "75 床",
    # 透析機。作者 2026-09-09 指定：不放機台數，改放機型與介紹。
    # 功能規格出自 hcmed.com.tw（華江醫療儀器）的公開產品頁：
    #   https://www.hcmed.com.tw/zh-tw/product/view-1
    # 但**掛名的是原廠日本 NIPRO，不是華江**——作者 2026-09-09 指定。
    # 華江是台灣代理商，病人看到的機器上印的是 NIPRO。
    # ⚠ 這是「原廠說這台機器做得到什麼」，不是院方公布的設備清單。
    # 中心實際使用的是否全數為這一型，要由院方確認。
    # 也刻意沒有抄產品頁的宣傳語（「最佳的醫療品質」那句）——
    # 醫療機構的網站不適合用最高級形容詞，只寫機器實際具備的功能。
    "machine":    "日本 NIPRO NCU-18",
    "machine_s":  "NCU-18",
    "staff":      "34 位",
    ...
    # 交通與接駁車出自郭綜合醫院官網的交通資訊頁：
    #   https://www.kgh.com.tw/Introduce/Item07
    # 那一頁的內容幾乎都在圖片裡（map.jpg 與 shuttle.jpg），文字版只有
    # 收費方式一句，所以是照著圖上的路線表與接駁車時刻整理的。
    #
    # ⚠ 但官網那張圖已經過期。2026-09-10 逐條查證現行路線（yunbus.tw 的
    # 路線 id 對得上台南市府 busmap 的 pathId，站名用官方英譯），結果：
    #   2   → 102（崑山科大－安平，起訖完全相同，仍停「郭綜合醫院」）
    #   5   → 105（都會公車，2026-08-21 上路，仍停「西門、民權路口」）
    #   7 / 14 / 18 / 綠17(G17) / 藍23(B23) / 藍24(B24) 編號不變，都仍停
    #   239 是高雄客運的跨市路線，不在台南市路線表裡，無法查證，故不列
    # 另外官網寫「從火車站搭 14」已經不對——現在 14 的起站是台南二中。
    # 102 本身就經過台南火車站又停醫院門口，是更好的答案。
    #
    # 簡化的重點：官網把九條路線的起站、停靠站、迄站全列出來，但要來看診
    # 的人只在意「哪一班到得了、下車走多遠」。所以拆成「直接到門口的」
    # 與「停在路口要走一段的」兩類，起訖站全部拿掉。
    "transit":    "公車 102 直接停「郭綜合醫院」站；"
                  "105、7、14、18、綠 17、藍 23、藍 24 停「西門、民權路口」",
    "parking":    "院區設有汽車與機車停車場，機車計次收費、汽車計時收費",
    # 接駁車的上車點正好就是透析中心那一棟（B 棟），對透析病人特別有用，
    # 所以獨立成一個欄位而不是揉進 transit。
    "shuttle":    "09:10、12:10、14:10、17:10",
    "shuttle_stop": "民生路 B 區大樓門口前",
...
/* 區塊裡第一個元素不要再帶自己的上緣 margin。
   section 的 padding-top 已經是「和上一區的距離」了，h2 的 44px 是
   「和上一段內文的距離」，兩個疊在一起就多出一截——about.html 開頭
   正好是 h2，量到 140px，其他頁都是 96px，多的 44 就是這個。
   （原本是在 education／services 用 style="margin-top:0" 各補一次，
   改成一條規則，新加的區塊不必記得補。） */
...
COLUMNS = [
    # 放第一個是刻意的：會點進衛教區的人，很多是剛被告知要開始透析、
    # 或家人剛開始透析。他們最想知道的是「第一天會發生什麼事」。
    ("第一次洗腎那天會發生什麼事", "col-first",
     "多數人是在還沒準備好的時候被告知要開始的。先把那一天會經過的流程"
     "說清楚，未知的部分少一點，就沒那麼可怕。"),
  ...
  <div class="split">
    ...
    <div{rv("", "right", .1)}>
      ...
      <ul class="facts">
        ...
        <li class="txt"><b>{fact('machine_s')}</b><span>透析機型</span></li>
...
以及心血管風險。下面是我們實際在做的事。</p>

<h3>每一次透析</h3>
<ul>
  <li>全程由腎臟專科醫師、資深護理師與醫療工程技師照護。</li>
  <li>監測血壓與症狀，記錄脫水量與透析過程中的變化。</li>
  <li>護理特殊小組施打 BioHole 無痛穿刺。</li>
  <li>不重複使用透析器（無 Reuse）；並設有專責隔離透析區。</li>
</ul>

<h3>定期追蹤</h3>
<ul>
  <li>抽血項目包含整套血球與生化檢查、各式肝炎、愛滋病、血液儲鐵蛋白與
  副甲狀腺素，用來評估透析效率、貧血、電解質與營養狀況，
  據以調整處方與用藥。</li>
  <li>每年固定安排胸部 X 光，以及腎臟、心臟、肝膽超音波。</li>
</ul>
...
<h3>初次透析</h3>
<p>第一次透析是整段治療裡最不安的時刻，多數人是在還沒準備好的情況下
被告知要開始的。所以第一次<strong>不會一開始就洗滿四小時</strong>——
時間會縮短、血流速度放慢，避免尿毒素清除太快引起透析不平衡症候群
（頭痛、頭暈、噁心）。接下來幾次再慢慢把時數拉長。</p>
<p>那天從報到、量體重、評估通路、上針到結束後的止血與休息，
每個環節都有人在旁邊看著；透析中有任何不舒服都可以當場說，
多數靠調整脫水速度或給藥就能緩解。廔管還沒成熟的人，
會先放暫時性的雙腔導管，等成熟再換過來。</p>
<p><a href="education.html#col-first">第一次洗腎那天會發生什麼事 →</a>
　完整流程、可能的不適、回家之後要注意的事，都寫在這一篇。</p>
...
def build_education() -> str:
    arts = {
        # 內容參考國內各大醫院公開的「初次血液透析病人須知」類衛教資料
        # （台大新竹分院、中榮、北榮護理部等），2026-09-09 整理。
        # 只寫各家都一致、屬於標準做法的部分；各院不同的細節（例如止血
        # 時間、紗布留置多久）一律寫成「依透析室的指示」，不替本中心
        # 訂一個數字——那是護理常規，要由中心自己說。
        "col-first": [
            "<p>多數人不是計畫好才開始透析的。可能是門診追蹤到一個階段，"
            "醫師說「差不多該準備了」；也可能是因為喘、水腫或高血鉀住進來，"
            "隔天就要洗第一次。無論是哪一種，第一次都會讓人很不安——"
            "而不安有很大一部分來自不知道會發生什麼。</p>",
            "<p>這篇就是把那一天會經過的事講清楚。</p>",

            "<h3>第一次通常不會洗滿四小時</h3>",
            "<p>長期透析多半是每週三次、每次四小時，但<strong>第一次不會"
            "一開始就洗這麼久</strong>，通常會縮短，血流速度也會放慢。</p>",
            "<p>這不是敷衍，是刻意的。開始透析之前，尿毒素已經在體內累積很久，"
            "如果第一次就大量清除，血液裡的滲透壓會降得比腦組織快，"
            "水分因此往腦細胞跑，造成輕微的腦水腫——這叫"
            "<strong>透析不平衡症候群</strong>，症狀是頭痛、頭暈、噁心、"
            "想吐，偶爾會視力模糊或特別煩躁，多半出現在透析中或結束後"
            "一兩小時內。</p>",
            "<p>把第一次的時間縮短、速度放慢，就是為了讓身體有緩衝。"
            "接下來幾次會慢慢把時間拉長到標準時數。<strong>等身體適應之後，"
            "該洗的時數還是要洗滿</strong>——長期洗不夠的後果，比一時的不舒服"
            "嚴重得多。</p>",

            "<h3>那天在透析室會經過什麼</h3>",
            "<ul>"
            "<li>到院後先量體溫、洗手，接著<strong>秤體重</strong>——"
            "這個數字決定這次要脫多少水，所以每次都要在同樣的條件下量。</li>"
            "<li>護理師評估血管通路、量血壓。</li>"
            "<li>通路消毒、上針，開始透析。過程中會反覆量血壓、"
            "問你有沒有不舒服。</li>"
            "<li>結束後拔針、加壓止血，再量一次血壓與體重。</li>"
            "<li><strong>不會馬上讓你走。</strong>要坐著休息一段時間，"
            "確認沒有頭暈、止血完全，才離開。</li>"
            "</ul>",
            "<p>止血要壓多久、紗布留多久，各家的常規不太一樣，"
            "請依照透析室當場的指示。</p>",

            "<h3>廔管還沒好，怎麼洗？</h3>",
            "<p>動靜脈廔管做完之後要幾週到幾個月才能用。如果來不及等，"
            "醫師會先在頸部、鎖骨下或鼠蹊部放一條<strong>暫時性的雙腔導管</strong>，"
            "等廔管成熟再換過來。</p>",
            ...
            "有些人會覺得胸悶、冒冷汗或皮膚癢。<strong>這些都要當場說，"
            "不要忍。</strong>多數可以靠調整脫水速度、暫停一下或給藥就緩解，"
            "忍到最後反而更難處理。</p>",
            "<p>不平衡症候群通常在<strong>第一週的適應期過後就不再出現</strong>。"
            "隨著體內尿毒素降下來、身體也習慣了，後面幾次會愈來愈平順。</p>",

            "<h3>回家之後</h3>",
            "<ul>"
            "<li><strong>先休息。</strong>剛回到家覺得累是正常的。頭暈就躺下來，"
            "不要勉強走動，免得跌倒。</li>"
            "<li><strong>不要因為口渴就大量喝水。</strong>下一次要脫的水就是"
            "這期間累積的。體重該控制在什麼範圍，看"
            "<a href=\"education.html#col-fluid\">兩次透析之間，體重可以增加多少？</a></li>"
            "<li><strong>絕對不要吃楊桃。</strong>楊桃裡的神經毒素腎臟排不掉，"
            "透析病人吃了可能引起打嗝不止、意識混亂甚至抽搐。這一項沒有"
            "「吃一點應該還好」。</li>"
            "<li><strong>藥照醫師開的吃。</strong>不要自己加減或停藥，"
            "也不要吃來路不明的中藥、草藥或保健食品——很多成分要靠腎臟排除。</li>"
            "<li><strong>摸一下廔管。</strong>震顫還在不在。</li>"
            "</ul>",

            "<h3>這些情況不要等到下次透析</h3>",
            "<ul>"
            "<li>頭痛得厲害、一直吐，或不舒服超過四到六小時沒有改善</li>"
            "<li>意識變得混亂，或發生抽搐</li>"
            "<li>穿刺處一直滲血止不住</li>"
            "<li>廔管的震顫變弱或摸不到</li>"
            ...
            "透析是一件會持續很多年的事，前面幾次本來就是在互相適應——"
            "你適應機器，我們也在調整你的處方。有任何不舒服或不懂的地方，"
            "當場問，不要自己猜。</p>",
        ],
        "col-diet": [
            "<p>透析病人最常被交代的是「這不能吃、那不能吃」，"
            "但門診裡真正常見的問題不是吃太多，是<strong>該有的量沒到，"
            "而吃進去的東西又不夠好</strong>。</p>",
            "<h3>蛋白質：量要夠，但要挑</h3>",
            "<p>透析過程本身會流失胺基酸，所以透析病人需要的蛋白質不比一般人少。"
            "長期攝取不足會造成肌肉流失與營養不良，而營養不良對長期存活的影響，"
            "比偶爾一次血磷偏高大得多。</p>",
            "<p>但「量要夠」不等於「多吃就好」。同樣的份量，來源不一樣，"
            "身體真正拿到的東西差很多——<strong>優先選高生物價的蛋白質</strong>"
            "（魚、蛋、瘦肉、豆製品），它們的胺基酸組成接近人體需要，"
            "利用率高，代謝出來的廢物相對少。加工肉品、香腸、火鍋餃類則是"
            "另一回事：蛋白質不見得比較多，磷和鈉卻高得多。</p>",
            "<p>一句話：<strong>量要夠，但要挑品質，不是靠份量堆上去。</strong></p>",
            "<h3>磷的重點不是總量，是來源</h3>",
...
院區旁邊，下車走幾步就到。<strong>從台南火車站過來也是搭 102</strong>——
它經過火車站，可以一路坐到醫院門口，不必轉車也不用走。</p>
<p>其餘路線停在<strong>「西門、民權路口」</strong>：105、7、14、18、
綠 17、藍 23、藍 24。從那一站下車後，往民生路二段方向再走一小段。</p>
<p class="tnote">102 與 105 是 2026 年改號的（原本的 2 號與 5 號），
如果看到舊的路線圖還寫 2 或 5，指的是同兩條線。另外台南火車站的公車
站位自 2026 年 3 月起因施工調整到成功路與中山路，搭車前留意一下。</p>

<h3>成大醫院接駁車</h3>
<p>院方有郭綜合⇄成大醫院的接駁車，<strong>郭綜合這一側的上車地點是
{fact('shuttle_stop')}，也就是透析中心所在的那一棟</strong>。</p>
<p>發車時間 {fact('shuttle')}，週一至週五行駛，
<strong>週六、週日與國定假日停駛</strong>。</p>

<h3>停車</h3>
<p>{fact('parking')}。機車停車場在民生路二段、復健中心旁邊，
汽車停車場在民生路二段的對側。</p>

<p class="tnote">接駁車與停車資訊整理自郭綜合醫院官網的
<a href="https://www.kgh.com.tw/Introduce/Item07" target="_blank"
rel="noopener">交通資訊頁</a>；公車路線於 2026 年 9 月逐條核對過現行編號。
路線與班次仍可能調整，出發前可到
<a href="https://2384.tainan.gov.tw/" target="_blank"
