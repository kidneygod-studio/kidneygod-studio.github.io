...
/* 導覽最緊的一階。門檻 2026-09-05 從 365 放寬到 440：
   ...
   都吃到這一階，六項才排得下一行。
   垂直內距一律不動，點擊區高度維持 42px。

   ⚠ 這一階**只收導覽，不動標誌**。第一版連 .brand 一起縮，結果 440px 時
   標誌被縮到 12.8px／74px 寬，中間卻空著 146px——導覽排不下是導覽的問題，
   標誌是被連坐的。標誌自己的縮小階仍維持在下面的 ≤365。 */
@media(max-width:440px){
  header.site nav a{padding:9px 2px;font-size:13px}
  header.site nav{gap:0}
}
/* 標誌最小的一階。維持 365：340–365 這段（多數 Android 的 360、部分 344）
   是兩階之間的漏網區，再往上放大就會和導覽擠在一起。 */
