...
PAGE_SOURCES: dict[str, list[str]] = {
    ...
    "high-potassium-ckd": ["kdigo_ckd", "tsn", "niddk"],
...
--accent-soft:#e3f0fa;--solid:#175c8e;--onsolid:#ffffff;
...
--line:#22303f;--card:#15202c;--accent:#7cc4ee;--accent2:#a9d9f7;--link:#8fd0f5;--warn:#fbbf24;--accent-soft:#122c3d;--solid:#7cc4ee;--onsolid:#0b1219}}
...
.dgj{background:var(--solid);color:var(--onsolid);font-size:11.5px;font-weight:700;
...
.nlj{font-size:11px;font-weight:700;letter-spacing:.06em;color:var(--onsolid);
background:var(--solid);border-radius:3px;padding:2px 7px;flex:0 0 auto}
...
.cats .d{font-size:14.5px;color:var(--mut);line-height:1.6}
...
.disclaimer{font-size:15px;color:var(--mut);background:var(--card);border-left:3px solid var(--warn);
...
border:0;border-radius:10px;background:var(--solid);color:var(--onsolid);cursor:pointer;white-space:nowrap}
...
.gg-sub{font-size:14.5px;color:var(--mut);line-height:1.65;margin:-6px 0 12px}
...
/* 2026-09-06 區塊間距 44px→26px。標題自帶左邊的色條，區塊分界靠那條線
   就看得出來，不需要再靠大片留白；44px 在手機上等於每捲一區就少半屏。 */
.sect{font-size:1.15rem;margin:26px 0 4px}
/* 緊接在開場白後面的第一個區塊再更近一點：開場白與第一個區塊之間
   本來就有搜尋框當視覺句點，不必再加一次分界。 */
.hero + .sect{margin-top:14px}
...
.mcard .d{font-size:14.5px;color:var(--mut);line-height:1.65}
...
.feat .d{font-size:15px;color:var(--mut);line-height:1.65}
...
/* 上方留白收緊（2026-09-26 作者反映留白太多）。從視窗頂端到站名原本 225px，
   四個來源各扣一點：band 的上內距 38、字級列的上外距 12、報頭的上內距 30，
   以及 **h1 從全站規則繼承來的 28px 上外距**（最後那個最容易漏看——
   它不是這份 CSS 寫的，是站名改用 h1 之後才被帶進來的）。
   這段只在首頁載入，不影響其他頁面。 */
main .band{padding-top:14px}
.prefbar{margin-top:4px}
.mg-head{padding-top:14px}
.mg-name{margin-top:0}
...
.mg-cover img{width:100%;height:auto;aspect-ratio:16/9;object-fit:cover;display:block;
...
.mg-pair img{width:100%;height:auto;aspect-ratio:16/9;object-fit:cover;display:block;
...
UPDATES_REVIEWED = "2026-09-10"

# (機構, 說明, [(標題, 年, 出處, 網址, 白話一句, 專業重點)])
GUIDELINES: list[tuple[str, str, list[tuple[str, str, str, str, str, str]]]] = [
    # 本土指引排第一：讀者在台灣就醫，健保給付、轉診時機、
    # 本地的病因結構都和歐美不同，這一份才是實際會被拿來用的那一份。
    # 全部連到學會的指引專區，不用 PDF 直連——那些路徑帶 UUID，
    # 學會重新上傳就會爛，而專區頁的網址十年不會變。
    ("台灣腎臟醫學會", "台灣本土的臨床指引與共識。國際指引是共同的基準，"
                        "但健保給付、轉診時機與本地的病因結構要看這一份。"
                        "主指引排在最前面，其餘依年份由新到舊；"
                        "全部都在學會的指引專區可以取得。", [
        ("台灣慢性腎臟病臨床診療指引", "2025 更新版",
         "台灣腎臟醫學會發行（2025 更新版 v1203），"
         "取代 2015 年由國家衛生研究院發行的版本",
         "https://www.tsn.org.tw/guide.html",
         "台灣自己的慢性腎臟病指引。國際指引講的是「一般原則」，"
         "這一份講的是**在台灣實際怎麼做**。",
         "2025 年更新，發行單位由國家衛生研究院（2015 年版）改為"
         "台灣腎臟醫學會；學會的指引專區另附一份使用說明文件。"),
        ("臺灣慢性腎臟病環境影響之全面性評估", "2026",
         "台灣腎臟醫學會",
         "https://www.tsn.org.tw/guide.html",
         "把環境因素對腎臟的影響整理成一份評估。"
         "台灣的腎臟病負擔特別重，環境暴露是被討論很久、"
         "但一直缺乏系統整理的一塊。",
         "2026 年發行。屬於評估報告而非診療指引，"
         "用途偏向政策與研究方向的盤點。"),
        ("台灣腎臟病患疫苗接種建議指引", "2026",
         "台灣腎臟醫學會",
         "https://www.tsn.org.tw/guide.html",
         "腎功能不好的人免疫力較差，感染的後果也比較嚴重，"
         "哪些疫苗該打、什麼時候打，這一份講得最清楚。",
         "2026 年發行。台灣本土的腎臟病患疫苗接種建議，"
         "涵蓋慢性腎臟病、透析與移植族群。"),
        ("腎移植患者心血管疾病臨床處置共識", "2026",
         "台灣腎臟醫學會、台灣移植醫學學會、中華民國心臟學會共同制定",
         "https://www.tsn.org.tw/guide.html",
         "移植之後心血管疾病是主要的死因之一。"
         "這份是腎臟科、移植科與心臟科三方一起訂的處置共識。",
         "2026 年由三個學會共同制定，"
         "跨科別共識通常代表這個問題單一專科處理不完。"),
        ("臺灣非典型溶血性尿毒症候群患者腎臟移植處置共識", "2025",
         "台灣腎臟醫學會、台灣兒童腎臟醫學會、台灣移植醫學學會共同制定",
         "https://www.tsn.org.tw/guide.html",
         "非典型溶血性尿毒症候群（aHUS）是罕見疾病，"
         "移植後有復發風險。這份處理的是這群人的移植決策。",
         "2025 年由三個學會共同制定，含兒童腎臟醫學會——"
         "aHUS 相當比例在兒童期發病。"),
        ("早期慢性腎臟病照護手冊", "2022",
         "國民健康署、台灣腎臟醫學會",
         "https://www.tsn.org.tw/guide.html",
         "針對**早期**慢性腎臟病的照護手冊。"
         "早期是最有機會延緩惡化的階段，卻也最容易被當成「還好」。",
         "2022 年由國民健康署與台灣腎臟醫學會共同發行，"
         "定位偏向照護實務與衛教，不是診療指引。"),
        ("腹膜透析治療臨床案例解析", "2021",
         "台灣腎臟醫學會（中文版）",
         "https://www.tsn.org.tw/guide.html",
         ...
         "比條列式的指引更容易看懂實務上怎麼判斷。",
         "2021 年發行中文版。案例式教材，"
         "適合搭配 KDOQI 與 KDIGO 的原則性建議一起看。"),
        ("台灣急性腎損傷處置共識", "2020",
         "台灣腎臟醫學會",
         "https://www.tsn.org.tw/guide.html",
         "急性腎損傷是可以恢復的，但處理的時機很關鍵。"
         "這份是台灣本土的處置共識。",
         "2020 年發行。急性腎損傷與慢性腎臟病的處置邏輯不同——"
         "前者找原因、爭取恢復，後者延緩惡化。"),
        ("人工腎臟重複使用指引參考資料", "未標示年份",
         "台灣腎臟醫學會",
         "https://www.tsn.org.tw/guide.html",
         "透析用的人工腎臟（透析器）能不能重複使用、怎麼做才安全，"
         "這份是相關的參考資料。",
         "學會的指引專區未標示年份。定位是參考資料而非正式指引，"
         "引用時要留意版本。"),
    ]),
    # 三高的本土指引由心臟／高血壓／血脂學會制定，不在腎臟醫學會那邊。
    # 這個站談的是「腎臟與三高」，兩邊都要收。
    ("台灣心臟與血脂學會", "三高的本土指引。台灣的高血壓與血脂標準"
                            "和歐美有差異，健保給付也不同，實務上以這兩份為準。", [
        ("台灣高血壓管理指引", "2022",
         "中華民國心臟學會（TSOC）與台灣高血壓學會（THS）共同制定；"
         "Acta Cardiol Sin 2022;38:225–325",
         "https://pubmed.ncbi.nlm.nih.gov/35673334/",
         "台灣自己的高血壓指引。它把高血壓的定義訂在 **130/80**，"
         "而且要求用**居家血壓**判斷——這正是本站〈722 原則〉那篇的依據。",
         "高血壓定義與治療目標皆為 <130/80 mmHg，"
         "且以標準化居家血壓（722 原則）為判斷基準，而非診間單次量測；"
         "用藥策略採 S-ABCDE 架構。"),
        ("台灣血膽固醇管理臨床路徑共識", "2025",
         "由九個學會與健保署共同制定，含台灣腎臟醫學會；"
         "Acta Cardiol Sin 2025;41(2)，"
         "DOI: 10.6515/ACS.202503_41(2).20250215A",
         "https://doi.org/10.6515/ACS.202503_41(2).20250215A",
         "台灣的膽固醇管理臨床路徑。重點不是「膽固醇要多低」，"
         "而是**不同風險的人各自該低到多少**，以及怎麼追蹤。",
         "2025 年 3 月發表。由心臟學會、血脂及動脈硬化學會、內科醫學會、"
         "腦中風學會、家庭醫學會、糖尿病學會、**腎臟醫學會**等九個學會"
         "與健保署共同制定；以 ASCVD 風險分層決定 LDL-C 目標，"
         "目的是讓各院所的作法一致。"),
    ]),
    ("KDIGO", "國際腎臟病預後改善組織。腎臟科最主要的國際指引來源，"
        ...
        ("慢性腎臟病的血壓處置", "2021",
         ...
         "且以標準化診間量測為前提。"),
        ("慢性腎臟病的礦物質與骨病變（CKD-MBD）", "2017 更新版",
         "Kidney Int Suppl 2017;7(1)",
         "https://kdigo.org/guidelines/ckd-mbd/",
         "磷、鈣與副甲狀腺素三個數字的處置依據。最值得注意的是它"
         "**刻意不給固定目標值**——只說把偏高的磷「朝正常範圍降」，"
         "而不是壓到某個數字。",
         "2017 年為針對特定章節的**局部更新**，未修訂的部分仍沿用 2009 年版。"
         "主要變動：磷由「維持在正常範圍」改為「朝正常範圍降」，"
         "並強調依持續或進行性升高決定治療、不對正常值做預防性處置；"
         "限制含鈣磷結合劑劑量的建議由特定族群擴大到"
         "**所有接受降磷治療的成人**；飲食建議納入磷的來源"
         "（動物性／植物性／添加物）吸收率差異。"),
    ]),
    ...
    ("AHA／ACC", "美國心臟協會與美國心臟病學會。高血壓標準的主要來源之一。", [
        ("心血管—腎臟—代謝症候群（CKM）的預防、偵測、評估與處置", "2026",
         "AHA／ACC／ADA／ASN 四學會共同制定；"
         "Circulation 與 JACC 同步發表（2026 年 6 月 9 日），"
         "DOI: 10.1161/CIR.0000000000001453",
         "https://doi.org/10.1161/CIR.0000000000001453",
         "**第一份把心臟、腎臟與代謝當成同一件事來寫的指引。**"
         "心臟科、腎臟科、糖尿病科、腎臟醫學會四邊一起訂——"
         "因為這三件事在同一個人身上本來就分不開。",
         "取代並擴充 2013 年的 AHA/ACC/TOS 肥胖處置指引；"
         "提出 CKM 症候群的分期架構，強調更早辨識風險；"
         "美國腎臟醫學會（ASN）是共同制定單位之一。"),
    ...
    ("ESC", "歐洲心臟學會。歐洲的標準，和美國版偶爾有差異，值得並列著看。", [
        ...
        ("血脂異常的處置（2019 版的 2025 重點更新）", "2025",
         "ESC 與歐洲動脈硬化學會（EAS）共同制定；"
         "Eur Heart J 2025;46(42):4359，同步發表於 Atherosclerosis",
         "https://pubmed.ncbi.nlm.nih.gov/40878289/",
         "歐洲的血脂指引更新。**風險評估工具換了**——"
         "從舊的 SCORE 改用 SCORE2／SCORE2-OP，"
         "同樣的膽固醇數字，算出來的風險可能和以前不一樣。",
         "為 2019 年版的重點更新，納入 2019 年之後至 2025 年 3 月 31 日的"
         "證據；風險評估改採 SCORE2（<70 歲）與 SCORE2-OP（≥70 歲），"
         "取代原本的 SCORE。"),
        ("血壓升高與高血壓的處置", "2024",
...
.fcard .fmeta{color:var(--mut);font-size:14.5px;margin:0 0 10px}
...
border:1px solid var(--solid);background:var(--solid);color:var(--onsolid);cursor:pointer}
