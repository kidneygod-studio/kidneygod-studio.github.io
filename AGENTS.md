...
這個 repo 由作者（吳政哲醫師）、**Claude Code**、**Codex** 編輯，另外還有
**排程機器人每天自動寫入**。這份檔案是規則，不是建議。

**2026-09-24 起分工改成這樣**（作者兩次指示的合併結果）：

| 誰 | 範圍 |
|---|---|
| Claude Code | 這個 repo 的**所有工作**——內容、程式、建置、發布、規則、紀錄 |
| Codex | **只做長文配圖**（`hero_src/<slug>.*`），見第六節 |

原本 09-24 早上寫的是「Codex 完全退出」，當天作者又指示配圖維持給 Codex——
因為 **Claude Code 不會生圖**，那一格沒有別人能補。所以 Codex 沒有離開，
是把範圍縮到只剩圖。Codex 會自動讀這個檔；Claude Code 從 `CLAUDE.md` 轉過來
也是讀這一份。**只維護這一份，不要另外寫一份給自己看的。**

⚠ 程式與內容現在只有一個人在看：以前互相看對方的 commit 等於有第二雙眼睛，
現在 Codex 只碰圖，等於沒有了。所以 `check_site.py` 與 CHANGELOG 的
「踩到什麼坑」更重要——那是唯一會提醒下一輪的東西。
...
**三個站各自獨立的產生器、各自一套樣式**，內容也互不相干。
（2026-09-23 起，原本「不共用樣式、不互相連結」那條規則作者已取消。）

**互相連結是現況，不是誤加，不要移除：**

| 方向 | 位置 | 改哪裡 |
|---|---|---|
| 衛教站 → 美食通 | 抽屜選單、頁尾、食物查詢頁 | `build_site.py` 的 `FOOD_TOUR_*` |
| 美食通 → 衛教站 | 每頁頁尾、健康美食之旅的免責聲明 | 桌面來源的 `build_static.py` 的 `KG_*` |

衛教站連出去的那幾條用 `target="_blank"`；美食通連回來的是同分頁（回到主站是
正常的離開動作，不需要另開）。`/dialysis/` 目前還沒有任何對外連結。

樣式仍然**不要**去統一——那是三套獨立的設計，混用 CSS 變數會出事（見下）。
...
理由：長文的成本幾乎都在查證與改寫，而 **Codex 能生圖、Claude Code 不能**
（這是能力限制，不是分工偏好——09-24 一度把全部工作收給 Claude，
當天就因為這一點把圖退回給 Codex）。

**不要互相代勞**——Claude 不要拿現成圖或別篇的圖硬湊，Codex 不要順手改文字內容。

### 交接怎麼走

1. Claude 寫完長文、建置推上線（**不必等圖**），並在回覆裡附上一段
   **生圖 prompt**（英文、含構圖與下面那些禁止事項）
2. Codex 依那段 prompt 生圖，放進 `hero_src/<slug>.*`
3. 誰先接手誰跑「圖進來之後」那五支，**做的人要講一聲**

**沒有圖可以先上線**——`hero_for()` 只認檔案在不在，缺圖時卡片會用
同尺寸的漸層佔位塊（不是破圖，格線也不會亂），`og/` 則沿用站徽。
內容早點被索引比等一張圖重要。圖到了再放進 `hero_src/`、重跑一次就換上。

### 交圖規格（Codex 看這裡；Claude 寫 prompt 也照這個寫）

檔名 **必須等於 `articles_src/` 那篇的 slug**，副檔名 `.jpg/.png/.webp` 皆可
（`make_hero.py` 會統一轉成 `hero/<slug>.jpg`）。

- **16:9，至少 1600×900**，建議 2752×1536 這種等級——`og/` 是從原圖裁的，
  像素多一點比較銳利。
- **主體壓在畫面中間 60%**。分享預覽圖會中央裁成 1200×630，LINE 還會裁得
  更接近 1:1；主體偏邊就會被切掉。
- **圖上不要有文字。** 生圖工具的中文幾乎都是壞的，而且會被上面那個裁切切斷。
- **不要畫成動作教學或醫療示範**（運動那篇的教訓：內文得另外加註「非動作教學」）。
  不要出現可辨識的真人、病歷、檢驗單。
...
python bump_assets.py    # ⚠ 一定要跑：hero/ 與 og/ 在 sw.js 走快取優先且不看 ?v=
python check_site.py
```

**漏跑 `bump_assets.py` 的話，造訪過的人永遠看到舊圖，而且不會有任何錯誤訊息。**

### 其他要一起處理的

- 新文章預設**沒有**審閱日期，先上線沒關係（作者定案：內容早點被索引比較重要，
  「醫師審閱」這個聲明晚一點出現是誠實的）。建完站推上去之後跑：

  ```bash
  python request_review.py <slug>      # 寄信給作者：文章連結 ＋ 一鍵確認連結
  ```
